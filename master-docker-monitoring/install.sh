...
prepare_root() {
  ...
  mkdir -p "$ZABBIX_DIR" "$PHPIPAM_DIR" "$GRAFANA_DIR" "$UPTIME_KUMA_DIR"

  # Only directory traversal permissions are set here.
  # DO NOT recursively chown this tree.
  chmod 755 /opt "$APP_ROOT" "$ZABBIX_DIR" "$PHPIPAM_DIR" "$GRAFANA_DIR" "$UPTIME_KUMA_DIR"
...
set_management_permissions() {
  ...
  chmod 600 "$ZABBIX_DIR/.env" "$PHPIPAM_DIR/.env" "$GRAFANA_DIR/.env" "$UPTIME_KUMA_DIR/.env"

  chmod 755 "$APP_ROOT" "$ZABBIX_DIR" "$PHPIPAM_DIR" "$GRAFANA_DIR" "$UPTIME_KUMA_DIR"
}
...
start_stack() {
  ...
  say "Pulling images only for new/missing components: ${services[*]}"
